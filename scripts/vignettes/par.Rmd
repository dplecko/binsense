...
```{r fi-search, fig.align='center', fig.width=12, fig.height=4, fig.cap="Estimation of ATE values based on the two-stage EM approach.", fig.width=18, fig.height=4}
...
fi_grid <- grid_search(fi_grid, "em")
grid_multi_plt(fi_grid, "em")
```

```{r x-grid-save, echo=FALSE}
# obtain the three plots (agnostic, x, y)

p_agn <- grid_to_plt(fi_grid[fi_grid$type == "agnostic", ], "em",
                     "agnostic")
ggsave(file.path(root, "paper", "figures", "par-agnostic.png"),
       p_agn, width = 6, height = 4)

p_x <- grid_to_plt(fi_grid[fi_grid$type == "x", ], "em", "x")
ggsave(file.path(root, "paper", "figures", "par-x.png"), 
       p_x, width = 6, height = 4)

p_y <- grid_to_plt(fi_grid[fi_grid$type == "y", ], "em", "y")
ggsave(file.path(root, "paper", "figures", "par-y.png"), 
       p_y, width = 6, height = 4)
```
...
axs_dat <- axis_search(X, Y, R, fi_seq, type = "x", solver = "em",
                       nboot = nboot)[["df"]]
...
plot_dat <- axis_search(Xsim, Ysim, Rsim, fi_seq, "x", "em",
                        nboot = nboot)[["df"]]
```
