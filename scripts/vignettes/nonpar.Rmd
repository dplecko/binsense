...
```{r nonpar-agnostic, fig.align='center'}
...
```{r x-grid, fig.align='center', fig.width=6, fig.height=4}
...
ggsave(file.path(root, "paper", "figures", "nonpar-x.png"), 
       width = 6, height = 4)
```
...
for (i in cool_ord) {
  ...
  cat(sprintf("<div class='grid-item'><a href='javascript:showModal(\"%s\");'>\\(\\phi_{x_0} = %s\\) <br> \\(\\phi_{x_1} = %s\\)</a></div>", plot_file, fi_x0, fi_x1))
...
```{r y-grid, fig.align='center', fig.width=6, fig.height=4}
...
ggsave(file.path(root, "paper", "figures", "nonpar-y.png"), 
       width = 6, height = 4)
```
...
for (i in cool_ord) {
  ...
  # Print clickable grid item
  cat(sprintf("<div class='grid-item'><a href='javascript:showModal(\"%s\");'>\\(\\phi_{y_0} = %s\\) <br> \\(\\phi_{y_1} = %s\\)</a></div>", plot_file, fi_y0, fi_y1))
...
```{r, axis-search-ci, fig.align='center', fig.cap="Estimation of the ATE based on the non-parametric approach, x-specific in the direction of increasing missingness for X = 0.", fig.width=6, fig.height=4}
...
ggsave(file.path(root, "paper", "figures", "nonpar-x-axis-ci.png"), 
       width = 6, height = 4)
```

Importantly, in this analysis, we do not find a $\phi$-value that would explain away the positive causal effect estimate of obesity on survival.
...
```{r check-correctness, fig.align='center', fig.cap="Estimation of the ATE based on the non-parametric approach, x-specific in the direction of increasing missingness for X = 0, for a model with a known ground truth.", echo=FALSE, warning=FALSE, fig.width=6, fig.height=4}
...
ggsave(file.path(root, "paper", "figures", "nonpar-ground-truth.png"), 
       width = 6, height = 4)
```
