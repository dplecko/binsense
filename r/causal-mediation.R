...
CausalExplanation_TV <- function(data, X, W, Z, Y, x0, x1, 
  ...
  expse <- expse_x1 - expse_x0
  expse[2] <- sqrt(expse_x1[2]^2 + expse_x1[2]^2)
  ...
  cef <- list(TV = 100 * tv, 
              #DE = de, SE = se, ETT = ett, IE = ie, TE = te, 
              DE = 100 * nde, IE = 100 * nie, 
              # ExpSE_x1 = expse_x1, ExpSE_x0 = expse_x0,
              SE = 100 * expse)
...
TV_family <- function(tvd, dataset) {
  ...
  ggplot(df, aes(x = Measure, y = Value, fill = Measure)) + geom_col() +
    ...
    xlab("Pathway") + 
    ggtitle(TeX(paste0("Causal decomposition of mortality difference: ",
                       str_to_title(dataset), " dataset"))) +
    ylab("Mortality difference (%)")
